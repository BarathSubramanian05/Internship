...
const attendanceArray = Object.values(groupedByDate).map((dayObj) => {
  ...
  // ✅ NEW: special case for attendanceMap sessions with null inTime/outTime
  const allNullSessions = attendanceMap[dayObj.date]?.every(
    s => s.inTime === null && s.outTime === null
  );
  if (allNullSessions) {
    overallStatus = "Present";      // overall status
    return {
      date: dayObj.date,
      inTime: "--",
      outTime: "--",
  ...
  // Default return if not special case
  return {
    date: dayObj.date,
    inTime: firstInTime,
    outTime: lastOutTime,
